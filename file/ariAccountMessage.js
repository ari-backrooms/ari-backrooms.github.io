$(function(){
  ...
  promise.then(()=>{
    if (!getislogin() || !userName || !userPassword) {
      ...
      $('div#relcontent').append('<span>你可以选择 <button onclick="open(location.origin+\'gets.noneuser/login/\')">登录</button> 和 <button onclick="open(location.origin+\'gets.noneuser/register/\')">注册</button></span>');
    }
  })
  promise.then(()=>{
